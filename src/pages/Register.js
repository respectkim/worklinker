...
function Register() {
  ...
  const consultantComments = {
    인공지능학습데이터구축:
      "💡 [컨설턴트 코멘트] 이른바 '데이터 라벨링'으로 불리는 직무입니다. 고도의 코딩 지식보다는 꼼꼼함과 데이터 전처리 역량이 필수적입니다. 진입 장벽이 상대적으로 낮아 IT 분야로의 빠른 재취업을 희망하시는 분들께 가장 현실적이고 강력히 추천하는 분야입니다.",
    인공지능서비스기획:
      '💡 [컨설턴트 코멘트] 개발을 직접 하기보다는 AI 기술을 활용해 어떤 서비스를 만들지 설계하는 역할입니다. 프로그래밍 지식보다 기존에 쌓아오신 풍부한 사회 경험(영업, 경영, 제조 등)과 AI를 접목할 수 있어, 이전 경력을 십분 살릴 수 있는 매력적인 직무입니다.',
    빅데이터분석:
      '💡 [컨설턴트 코멘트] 수많은 데이터 속에서 의미 있는 패턴과 결과를 찾아내는 직무입니다. 파이썬이나 엑셀을 활용한 데이터 가공 및 시각화 역량이 필요합니다. 현재 모든 산업군에서 데이터 분석 역량을 요구하고 있어 취업 시장에서 수요가 매우 높습니다.',
    데이터엔지니어링:
      '💡 [컨설턴트 코멘트] 데이터가 원활하게 수집되고 저장될 수 있도록 인프라(뼈대)를 구축하는 직무입니다. 데이터 분석 자체보다는 서버 구축이나 데이터베이스(DB) 관리에 흥미가 있는 분들에게 적합한 시스템 기반 직무입니다.',
    인공지능모델링:
      '💡 [컨설턴트 코멘트] 딥러닝과 머신러닝 알고리즘을 직접 설계하고 AI를 학습시키는 심화 직무입니다. 수학적 사고력과 파이썬 프로그래밍에 대한 깊은 이해가 요구됩니다. 학습에 시간과 노력이 필요하지만, 그만큼 고도의 전문성을 인정받을 수 있습니다.',
    응용SW엔지니어링:
      '💡 [컨설턴트 코멘트] 우리가 흔히 사용하는 웹사이트나 스마트폰 앱을 개발하는 전통적인 소프트웨어 개발 직무입니다. 자바(Java)나 스프링(Spring) 등의 프로그래밍 언어와 프레임워크 활용 역량이 필수적으로 요구됩니다.',
  };
  ...
  const handleCheckId = async () => {
    ...
    try {
      ...
      if (response.data.ok) {
        ...
        alert(response.data.error || '이미 사용 중인 아이디입니다.');
        setIsIdVerified(false);
      }
    } catch (error) {
      ...
      alert(error.response?.data?.error || '서버 통신에 실패했습니다.');
      setIsIdVerified(false);
    }
  };
  ...
  const handleJobSelect = (jobName) => {
    setForm((prev) => {
      ...
      if (isSelected) {
        return {
          ...prev,
          selectedJobs: prev.selectedJobs.filter((item) => item !== jobName),
        };
      }
      ...
      return {
        ...prev,
        selectedJobs: [...prev.selectedJobs, jobName],
      };
    });
  };
        ...
        <form onSubmit={handleSubmit} className="register-form">
          ...
          <div className="form-group">
            ...
            <div className="phone-row">
              <input
                name="phone"
                placeholder="XXX-XXXX-XXXX 형태로 입력"
                value={form.phone}
                onChange={handleChange}
              />
            </div>
          ...
          <div className="form-group">
            ...
            {form.confirmPassword && (
              <span
                className={`password-match-msg ${
                  form.password === form.confirmPassword ? 'success' : 'error'
                }`}
              >
                {form.password === form.confirmPassword
                  ? '✅ 비밀번호가 일치합니다.'
          ...
          <div className="job-selection-wrapper">
            ...
            <div className="job-grid">
              {selectedCategory === 'IT_AI' ? (
                jobCategories
                  .filter((job) => job.category === selectedCategory)
                  .map((job) => (
                    <div
                      key={job.id}
                ...
                <div className="coming-soon-card">
                  ...
                  <p>
                    초기 서비스는 데이터가 가장 많이 확보된 <b>'IT·인공지능'</b> 분야를 중심으로 제공됩니다.
                    <br />
                    <br />
                ...
                <div key={jobName} className="feedback-item">
                  ...
                  <p>
                    <span className="dynamic-text">
                      {form.address
                        ? `입력하신 [ ${form.address.split(' ')[1] || '해당'} ] 지역을 중심으로 분석한 결과입니다. `
                        : ''}
                    </span>
          ...
          <div className="terms-container">
            ...
            <div className="term-list">
              <label className="term-item">
                <input type="checkbox" />
                <span>
                  [필수] 만 19세 이상입니다. <span className="star">★</span>
                </span>
              </label>
              <label className="term-item">
                <input type="checkbox" />
                <span>
                  [필수] 서비스 이용 약관 <span className="star">★</span>
                </span>
              </label>
              <label className="term-item">
                <input type="checkbox" />
                <span>
                  [필수] 개인정보 수집 이용동의 <span className="star">★</span>
                </span>
              </label>
            </div>
          </div>
